...
class User_Model extends Model
{
    ...
    static function findOrderStatusCountByUser($uid, array $status)
    {
        ...
        foreach ($status as $statu) {
            ...
            $condition .= "(SELECT count(1) c FROM shp_order_info where is_separate = 0 and is_delete=0 and user_id=$uid $where ) t$i ,";
        }
        $field = rtrim($field, ',');
    ...
    static function saveMerchantInfo($mobile, $inviteCode, $password, $uid = 0)
    {
        ...
        if ($admin_uid !== false) {
            ...
            $mch = Merchant::getMerchantByUserId($uid);
            if($mch->is_exist()){
                $insertarr['merchant_id'] = $mch->uid;
                $effnum = D()->update($tablename, $insertarr, ['merchant_id' => $mch->uid], 'IGNORE');
            }else{
