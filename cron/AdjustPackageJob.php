...
class AdjustPackageJob extends CronJob {
	...
	public function main($argc, $argv) {
        ...
        if($old_pack == 398 || $old_pack == 698){
            ...
            if($new_pack == 999){
                ...
                $mock->createOrder($user, '', $new_pack, Order::ORDER_FLAG_AGENT, ($new_pack == 698?GOLD_AGENT_GOODS_ID : SILVER_AGENT_GOODS_ID));
            }
            D()->commit();
            ...
            $this->removeMerchantRecords($uid);
	...
	private function createMerchant($user, $new_pack){
	    ...
	    $mock = new MockCreateMerchantJob();
        $mock->createOrder($user, $m->uid, $new_pack, Order::ORDER_FLAG_MERCHANT, MECHANT_GOODS_ID);
	}
